...
from src.llm.openrouter_engine import OpenRouterEngine
...
def _load_fixed_proxy_prompt() -> str:
    cfg = Path(__file__).resolve().parents[2] / "config" / "PROXY.md"
    if cfg.exists():
...
def _build_system_prompt(state: "ProxyState") -> str:
    ...
    if state.agent_history_text:
        parts.append("## Main Agent History (recent, text-only)")
        parts.extend(state.agent_history_text)
        parts.append("")

    if state.agent_history_full:
        parts.append("## Main Agent History (last N, full resolution)")
        parts.extend(state.agent_history_full)
        parts.append("")
...
class ProxyState:
    ...
    agent_history_text: list[str] = field(default_factory=list)
    agent_history_full: list[str] = field(default_factory=list)
...
class ProxyController:
    ...
    def get_context_stats(self) -> dict:
        prompt = _build_system_prompt(self.state)
        convo_chars = sum(len(m.get("content", "") or "") for m in self.conversation)
        return {
            "prompt_chars": len(prompt),
            "conversation_chars": convo_chars,
            "total_chars": len(prompt) + convo_chars,
            "proxy_md_chars": len(FIXED_PROXY_PROMPT),
            "compressed_chars": len(self.state.compressed_context or ""),
            "history_text_chars": sum(len(x) for x in self.state.agent_history_text),
            "history_full_chars": sum(len(x) for x in self.state.agent_history_full),
            "live_turn_chars": sum(len(t.get("content", "") or "") for t in self.state.agent_turns),
            "scratchpad_chars": len(self.state.scratchpad_task or ""),
            "queued_chars": len(self.state.queued_task or ""),
            "history_text_count": len(self.state.agent_history_text),
            "history_full_count": len(self.state.agent_history_full),
        }
    ...
    def update_agent_context(
        ...
        compressed_context: str = "",
        history_text: list[str] | None = None,
        history_full: list[str] | None = None,
        ...
        if history_text is not None:
            self.state.agent_history_text = history_text
        if history_full is not None:
            self.state.agent_history_full = history_full
    ...
    def reset(self):
        ...
        self.state.agent_history_text.clear()
        self.state.agent_history_full.clear()
        self.state.pending_completion_brief = ""
        self.state.must_brief_before_dispatch = False
